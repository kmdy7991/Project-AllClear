...
#### 모델 학습 V1 버전
 - 시설 작물 개체 이미지 ( 파프리카 볼로키 ) 데이터를 사용하기로 결정했다.
 - 데이터가 많아서 열매가 라벨링 되어있는 데이터만 학습시켜 보기로 했다.
 - 데이터의 라벨링 처리가 착과기, 수확기 구분 없이 "열매" 라는 라벨링으로 처리되어 있어서 라벨링을 새롭게 해줘야 한다고 깨달았다.

#### 모델 학습 V2 버전
 - V1 에서 라벨링 처리의 필요성을 깨닫고 "열매" 라벨링 데이터를 "착과기", "수확기" 2개의 라벨링으로 분리함.
 - 이미지의 크기를 리사이징해서 통일하고 회전시켜서 데이터를 추가하고 학습시킴.
 - 사진에 있는 열매를 잡지 못하거나 착과기와 수확기를 잘못 판단하는 문제 발생.
 - 정확도를 더 높여야 한다고 느낌.

#### 모델 학습 V3 버전
 - 이번엔 열매 뿐만이 아닌 모든 사진을 학습시켜 보기로 했음.
 - V2와 마찬가지로 이미지 크기를 리사이징하고 이미지 회전으로 데이터를 추가함.
 - "열매" 데이터에 비해 "잎", "줄기" 등의 데이터가 과하게 많은 문제를 발견함.
 - 줄기 부분을 열매로 착각하거나 열매 자체를 잘 잡지 못하는 문제가 발생
 - 데이터 상에서 수확이 가능한 걸로 보이는데 "착과기"로 라벨링 되어 있거나 반대로 덜 익은게 분명한데 "수확기"로 라벨링 된 데이터들을 발견해서 데이터를 한번 걸러줄 필요성을 느낌.

#### 모델 학습 V4 버전
 - 열매가 포함되어 있는 사진들을 전부 확인해서 수확 가능 여부를 판단하고 걸러주고 학습을 시킴.
 - 수확 가능의 기준은 착색이 잘 되었는지를 기준으로 했음.
 - 실제로 다 거르니까 수확 가능 사진이 250장 정도밖에 나오지 않음. 나머지 11000장은 전부 수확 불가.
 - 수확 가능과 불가능의 사진을 비슷한 비율로 맞추고 학습 시작.
 - 인식 자체가 잘 안되고 특정 사진만 높은 수치가 나오는 모습이 보임.

#### 모델 학습 V5 버전
 - 이번엔 학습 데이터를 바꿔서 사용해보기로 함. 사용한 학습 데이터는 지능형 스마트팜 통합 데이터(파프리카) 를 사용함.
 - 열매 데이터만 학습시킴
 - 해당 데이터에는 라벨링이 색깔 기준으로 되어 있었음. 초록색 / 초록색 이외의 색
 - 열매의 사진은 약 2.7 만장 정도 나오고 비율은 4 : 6 정도가 되었음.
 - V4와 마찬가지로 사진을 하나씩 확인해서 수확 가능인데 덜 익은 것 같은 사진들은 제외 시킴.
 - 학습 결과 파프리카는 잘 인식하고 수확 여부도 잘 파악할 수 있었음.
 - 문제는 착색이 덜 된 파프리카도 수확 가능으로 인식하는 경우가 있음.
 - 또한 전체적인 학습 데이터 부족으로 파프리카 외의 다른 객체도 파프리카로 잘못 인식하는 경우가 있음.
 - 최종적으로 화면에 파프리카만 보이면 괜찮게 수확 가능 여부를 판단할 수 있다고 생각해서 V5 모델을 프로젝트에 적용함.

#### Object Tracking 적용
 - 프로젝트의 특성상 파프리카를 인식하고 해당 파프리카의 위치 정보를 가져와야 하는데, 카메라가 움직이면 다른 파프리카를 인식하는 문제가 있었음.
 - 그래서 처음 인식한 파프리카만 계속 추적할 수 있도록 Object Tracking 기술을 적용함.
