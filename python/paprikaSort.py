...
# 필터링 데이터만 따로 저장
saveDir = "C:/SSAFY/ssafyPJT-final/dataset/Training/sort/"
saveLabelingDir = ""
saveImageDir = ""
...
for i in range(len(annoResampleList)):
    ...
    if step == "착과기":
        saveImageDir = saveDir + "type1/img"
        saveLabelingDir = saveDir + "type1/txt"
    elif step == "착색기":
        saveImageDir = saveDir + "type2/img"
        saveLabelingDir = saveDir + "type2/txt"
    elif step == "과실비대기":
        saveImageDir = saveDir + "type3/img"
        saveLabelingDir = saveDir + "type3/txt"
    elif step == "과비대성숙기":
        saveImageDir = saveDir + "type4/img"
        saveLabelingDir = saveDir + "type4/txt"
    elif step == "수확기":
        saveImageDir = saveDir + "type5/img"
        saveLabelingDir = saveDir + "type5/txt"
    ...
    new_file = open(os.path.join(saveLabelingDir, annoResampleList[i].split('.')[0] + '.txt'), "a")
    shutil.copy(baseDir + 'origin_paprika/' + image_name, saveImageDir)
